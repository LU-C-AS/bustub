...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (args.size() != 1 || args[0]->GetReturnType() != TypeId::VARCHAR) {
    throw Exception("args wrong\n");
  }
  std::shared_ptr<StringExpression> ptr;
  if (func_name == "upper") {
    ptr = std::make_shared<StringExpression>(args[0], StringExpressionType::Upper);
    return ptr;
  }
  if (func_name == "lower") {
    ptr = std::make_shared<StringExpression>(args[0], StringExpressionType::Lower);
    return ptr;
  }

  throw Exception("func call not supported in planner yet\n");
}
